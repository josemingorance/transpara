    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${apiUrl}/analytics/geographical_distribution/`
        );
            ...
            .map(([region, data]) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  >
                    <h4 className="font-semibold text-sm hover:text-blue-600">{region}</h4>
                    <p className="text-xs text-gray-500 mt-1">Click to filter table</p>
                  </div>
                  <div
                ...
                <button
                  onClick={() => {
                    router.push(`/contracts/filtered?region=${encodeURIComponent(region)}`);
                  }}
                  className="mt-4 w-full py-2 px-3 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded text-xs font-semibold transition-colors"
                >
                  📋 View Contracts →
                </button>
              </div>
          ...
          <table className="min-w-full border-collapse">
            ...
            <tbody>
              {filteredData.map((location, idx) => (
                <tr
                  key={idx}
                  className="hover:bg-blue-50 border-b transition-colors cursor-pointer"
                  onClick={() => {
                    router.push(
                      `/contracts/filtered?region=${encodeURIComponent(location.region)}&municipality=${encodeURIComponent(location.municipality)}`
                    );
                  }}
                  title="Click to view contracts for this location"
                >
